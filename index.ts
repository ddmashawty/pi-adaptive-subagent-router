...
import { resolveSubagentLaunchContract } from "../../npm/node_modules/pi-subagents/src/api/preflight.ts";
import {
	costScore,
	inferDuty,
	lowerCostCandidates,
...
const laneSchema = Type.Object({
	...
	agent: Type.String({ minLength: 1, description: "Subagent role: scout, reviewer, researcher, oracle, worker, or delegate." }),
	task: Type.String({ minLength: 1, description: "Narrow task contract for this lane." }),
	duty: Type.Optional(Type.String({ enum: ["scout", "reviewer", "oracle", "worker", "delegate", "research"], description: "Routing duty. Defaults from the agent name." })),
	...
	worktree: Type.Optional(Type.Boolean({ description: "Required for the single writing lane (worker or delegate); runs it in a managed Git worktree." })),
...
export default function adaptiveSubagentRouter(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Launch a risk-aware dynamically routed workflow for read-only lanes or one managed-worktree writing lane (worker/delegate). Balances task complexity, risk, lane duty, evidence gates, quality policy, and cost.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			await Promise.all(normalizedLanes.map(async (lane, index) => {
				const resolved = await resolveSubagentLaunchContract({
					agent: lane.agent,
					task: lane.task,
					cwd: lane.cwd ?? ctx.cwd,
					context: lane.context ?? (lane.duty === "oracle" || lane.duty === "worker" || lane.duty === "delegate" ? "fork" : "fresh"),
					model: routes[index]!.model,
					availableModels: models,
					preferredProvider: parent.provider,
					output: lane.output ?? false,
				});
				assertResolvedLaneContract(lane.duty, lane.agent, resolved);
			}));
